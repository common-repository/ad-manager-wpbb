...
function letsfxad_insertion_options_page() {
    ...
    <table>
        ...
        <tr><td>Blog Post Top Ad: </td><td style="border: solid gray 1px"><textarea rows="6" cols="40"  type="text" name="wp_letsfxad_top" ><?php echo get_option('wp_letsfxad_top'); ?></textarea></td><td style="border: solid gray 1px"><?php echo letsfx_fix_content(get_option('wp_letsfxad_top')); ?></td></tr>
        <tr><td>Blog Post Bottom Ad: </td><td style="border: solid gray 1px"><textarea rows="6" cols="40" type="text" name="wp_letsfxad_bottom" ><?php echo get_option('wp_letsfxad_bottom'); ?></textarea></td><td style="border: solid gray 1px"><?php echo letsfx_fix_content(get_option('wp_letsfxad_bottom')); ?></td></tr>
        <tr><td>BuddyPress Above: </td><td style="border: solid gray 1px"><textarea rows="6" cols="40" type="text" name="wp_letsfxad_other1" ><?php echo get_option('wp_letsfxad_other1'); ?></textarea></td><td style="border: solid gray 1px"><?php echo letsfx_fix_content(get_option('wp_letsfxad_other1')); ?></td></tr>
        <tr><td>BuddyPress Forum Topic Bottom: </td><td style="border: solid gray 1px"><textarea rows="6" cols="40" type="text" name="wp_letsfxad_other2" ><?php echo get_option('wp_letsfxad_other2'); ?></textarea></td><td style="border: solid gray 1px"><?php echo letsfx_fix_content(get_option('wp_letsfxad_other2')); ?></td></tr>
        <tr><td>Bottom floating Ad: </td><td style="border: solid gray 1px"><textarea rows="6" cols="40" type="text" name="wp_letsfxad_other3" ><?php echo get_option('wp_letsfxad_other3'); ?></textarea></td><td style="border: solid gray 1px"><?php echo letsfx_fix_content(get_option('wp_letsfxad_other3')); ?></td></tr>
    </table>
    <div class="submit">
...
$letsfxad_adsused_c = 0;
  ...
  function letsfxad_hide(){
      if(strpos($_SERVER['REQUEST_URI'],'pr_sponsored_post')!==false) return true;
      if ( is_user_logged_in() ){
        $letsfxad_hide = letsfx_fix_content(get_option('wp_letsfxad_hide'));
        global $current_user;
        get_currentuserinfo();
...
{       
    ...
    if(strpos($content,'<!--NOADD-->')!==false) return($content);
    
    global $letsfxad_adsused_c;
    $content.= '<!-- letsfxad_adsused_c= '.$letsfxad_adsused_c.'-->';
    $letsfxad_adsused    = letsfx_fix_content(get_option('wp_letsfxad_adsused'));
    if($letsfxad_adsused_c>$letsfxad_adsused)return($content) ;
    
    $top    = letsfx_fix_content(get_option('wp_letsfxad_top'));
    if(strlen($top)>2) $letsfxad_adsused_c++;
    //if($letsfxad_adsused_c>=$letsfxad_adsused)return(($top).$content) ;
    
    $bottom = letsfx_fix_content(get_option('wp_letsfxad_bottom'));
...
function bb_letsfxad_before_directory(  ) {
    ...
    $other1 = letsfx_fix_content(get_option('wp_letsfxad_other1'));
...
function bb_letsfxad_topic_footer( $content ) {
    ...
    $other2 = letsfx_fix_content(get_option('wp_letsfxad_other2'));           
...
function bb_letsfxad_activity( ) {
    if(letsfxad_hide()) return;
    global $letsfxad_adsused_c;
    $letsfxad_adsused    = (get_option('wp_letsfxad_adsused'));
...
function bb_letsfxad_float( ) {
    ...
    $letsfxad_adsused    = letsfx_fix_content(get_option('wp_letsfxad_adsused'));
    if($letsfxad_adsused_c>=$letsfxad_adsused)return ;
    $other3 = letsfx_fix_content(get_option('wp_letsfxad_other3'));
    ...
    <small style="background-color: #EDEFF4;color: #3B5998;z-index: 9999;float: left;position: absolute;display:inline ; cursor:pointer;bottom : 0px;left: 150px" onclick="document.getElementById('divadfloat').style.display='none'; letsfx_createCookie('letsfx_float',1,1)">[Close]</small>
    <?php echo $other3; ?>
    <script type="text/javascript">// <![CDATA[
        if(letsfx_readCookie('letsfx_float')==null)
        ...
        function letsfxad_scrl(){
            ...
            function letsfx_createCookie(name,value,days) {
                if (days) {
                    var date = new Date();
            ...
            function letsfx_readCookie(name) {
                var nameEQ = name + "=";
            ...
            function letsfx_eraseCookie(name) {
                createCookie(name,"",-1);
            }
    // ]]></script></div>     
    <?php                              
...
function letsfx_fix_content($content){
